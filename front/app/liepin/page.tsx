...
export default function LiepinPage() {
          ...
          handler: (event) => {
            try {
              ...
              if (data.platform === 'liepin') {
                console.log('[猎聘 SSE] connected事件数据:', data)
                console.log('[猎聘 SSE] liepinLoggedIn状态:', data.linpinLoggedIn)
  ...
  const handleOpenLiepinLogin = async () => {
    try {
      const response = await fetch('http://localhost:8888/api/liepin/login', { method: 'POST' })
      const data = await response.json()
      if (!data.success) {
        console.warn('打开猎聘登录页失败：', data.message)
      }
    } catch (error) {
      console.error('Failed to open Liepin login page:', error)
    }
  }
        ...
        actions={
          <div className="flex items-center gap-2">
            {checkingLogin ? (
              ...
              <Button onClick={handleOpenLiepinLogin} size="sm" className="rounded-full bg-gradient-to-r from-teal-500 to-green-500 hover:from-teal-600 hover:to-green-600 text-white px-4 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                <BiPlay className="mr-1" /> 登录猎聘
              </Button>
            ) : isDelivering ? (

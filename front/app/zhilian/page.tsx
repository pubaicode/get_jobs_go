...
export default function ZhilianPage() {
  ...
  const handleOpenZhilianLogin = async () => {
    try {
      const response = await fetch('http://localhost:8888/api/zhilian/login', { method: 'POST' })
      const data = await response.json()
      if (!data.success) {
        console.warn('打开智联招聘登录页失败：', data.message)
      }
    } catch (error) {
      console.error('Failed to open Zhilian login page:', error)
    }
  }
        ...
        actions={
          <div className="flex items-center gap-2">
            {checkingLogin ? (
              ...
              <Button onClick={handleOpenZhilianLogin} size="sm" className="rounded-full bg-gradient-to-r from-teal-500 to-green-500 hover:from-teal-600 hover:to-green-600 text-white px-4 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
                <BiPlay className="mr-1" /> 登录智联招聘
              </Button>
            ) : isDelivering ? (
